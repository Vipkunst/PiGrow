...
namespace PiGrow.Services
{
    public class ConditionCheckerService : BackgroundService
    {
        ...
        {
            if (!TryGetSensorValue(HumidityTopic, out double humidity))
            {
            ...
            {
                _logger.LogInformation("Humidity {Value:F1}% below min {Min}% → pump ON", humidity, _humidityThreshold.Min);
                return true;
            }

            if (humidity >= _humidityThreshold.Max)
                _logger.LogInformation("Humidity {Value:F1}% at or above max {Max}% → pump OFF", humidity, _humidityThreshold.Max);
